...
def generate_story(req: StoryRequest):
    ...
    system_prompt = (
        ...
        "- MULTI-CHARACTER CAMERA: when two or more characters appear together, the CAMERA AND DISTANCE must ALWAYS be a wide shot with full bodies visible — never 'medium wide shot' or 'from knees up'. State the physical gap between them explicitly, for example 'standing two meters apart'. Close framing causes the characters' faces and clothing to blend together, which is a failure.\n"
        "- Each character must be described in its OWN separate sentence. Never describe two characters in one sentence. State the gender word ('boy' or 'girl') for each one explicitly, every time.\n\n"
        "IMAGE PROMPT RULES:\n"
        "Every image_prompt must follow this exact structure:\n"
        "\"Children's picture book illustration, soft hand-painted watercolor, gentle pastel palette, soft brush edges, flat 2D, no outlines. [CAMERA AND DISTANCE]. [CHARACTER DESCRIPTION VERBATIM] is [DESCRIBE THE ACTION IN PHYSICAL DETAIL: body position, limbs, facial expression, what their hands are doing]. [DESCRIBE WHAT IS HAPPENING IN THE SCENE AROUND THEM: where the creature/object is, what it is doing, how it relates to the character]. In the background: [AT LEAST 3 SPECIFIC ENVIRONMENT DETAILS FROM THE STORY]. The character is small relative to the scene. Absolutely no text, no letters, no words, no writing, no signatures, no watermarks anywhere in the image.\"\n"
        "LOCKED PHRASES: every image_prompt must BEGIN with exactly 'Children\'s picture book illustration, soft hand-painted watercolor, gentle pastel palette, soft brush edges, flat 2D, no outlines.' and must END with exactly 'Absolutely no text, no letters, no words, no writing, no signatures, no watermarks anywhere in the image.' Reproduce both word-for-word on every page. Never reword, shorten or vary them — the art style must be identical across all pages of the story.\n\n"
        ...
        "- POSE RULE: characters must be standing, sitting upright, kneeling, crouching, walking or running — always seen from a normal upright viewpoint. NEVER lying down, NEVER reclining, NEVER sprawled on their back, NEVER seen from above their head or feet. Reclining and foreshortened poses produce broken anatomy — missing legs, extra arms, fused limbs — and are a failure.\n"
        "- Every character must have their full body clearly readable: two arms, two legs, in a natural position. If the story text describes lying down, illustrate the moment just before or after instead — sitting up, settling down, or resting against something upright.\n\n"
